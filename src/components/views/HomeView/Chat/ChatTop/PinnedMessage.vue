...
const handleHidePinnedMessage = () => {
  if (props.activeConversation) {
    ...
    let activeConversationIndex = getConversationIndex(props.activeConversation.id);
...
<template>
  <SlideTransition animation="shelf-down">
    <div
      ...
      v-if="
        props.activeConversation.pinnedMessage &&
        !props.activeConversation.pinnedMessageHidden
      "
    >
      <!--pinned message preview-->
      <MessagePreview :message="props.activeConversation?.pinnedMessage" />
