...
const sendTextMessage = async () => {
  if (textValue.value.trim() !== "") {
    try {
      await apiModule.messages.send({
        chatId: props.activeConversation.id,
...
<template>
  <div class="w-full">
    ...
    <div
      ...
      :class="{ 'pt-[3.75rem]': props.activeConversation?.replyMessage }"
    >
      <ReplyMessage :active-conversation="props.activeConversation"/>
    </div>
      ...
      <div class="grow md:mr-5 xs:mr-4 self-end" v-if="!recording">
        <div class="relative">
          <Textarea
            ...
            :value="textValue"
            @value-changed="(value: string) => (textValue = value)"
            auto-resize
            cols="30"
            rows="1"
