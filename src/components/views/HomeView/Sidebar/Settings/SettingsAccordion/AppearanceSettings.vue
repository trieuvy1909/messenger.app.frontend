...
<template>
  ...
  <Collapse id="appearance-settings-collapse" :collapsed="props.collapsed">
    <SettingsSwitch
      ...
      :value="!!store.user.settings.darkMode"
      :handle-toggle-switch="(value) => (store.user.settings.darkMode = value)"
      class="mb-7"
    />
    <SettingsSwitch
      title="Bordered Theme"
      description="Apply borders to the theme"
      :value="!!store.user.settings.borderedTheme"
      :handle-toggle-switch="(value) => (store.user.settings.borderedTheme = value)"
      class="mb-7"
    />
  </Collapse>
</template>
